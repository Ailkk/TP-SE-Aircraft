...
public class AirSimulation
{
   ...
   public void agent3() throws InterruptedException
   {
      ...
      for(int i = 0; i<(this.a.getNumberOfRows()*this.a.getSeatsPerRow()); i++) {
    	  //random choice for seat1
    	  int row1 = R1.nextInt(this.a.getNumberOfRows());
    	  int col1 = R1.nextInt(this.a.getSeatsPerRow());
    	  
    	  //random choice for seat2
    	  int row2 = R2.nextInt(this.a.getNumberOfRows());
    	  int col2 = R2.nextInt(this.a.getSeatsPerRow());
    	  
    	  System.out.println("Passsage numero " + i);
    	  //checking if both seats aren't empty (otherwise we don't have to check anything...)
    	  if(!this.a.isSeatEmpty(row1, col1) && !this.a.isSeatEmpty(row2, col2)) {
    		  
    		  Customer c1 = (Customer) this.a.getCustomer(row1, col1).clone();
    		  Customer c2 = (Customer) this.a.getCustomer(row2, col2).clone();
    		  
    		  //if a customer has highest flying level than another, switch both customers
    		  if((c1.getFlyerLevel() < c2.getFlyerLevel()) || (c1.getFlyerLevel() > c2.getFlyerLevel())) {
    			  this.a.freeSeat(row1, row1);
